...
class ChatDOMWatcher {
     ...
     */
    constructor(cdpPort = 9222, callback, pollIntervalMs = 400) {
    ...
    async readLastMessage() {
        const unknown = {
            category: "UNKNOWN" /* LastMessageCategory.UNKNOWN */,
            text: "",
            confidence: 0,
        };
    ...
    static _classify(text) {
        ...
        if (WAIT_PAT.test(text)) {
            return {
                category: "WAITING_FOR_INPUT" /* LastMessageCategory.WAITING_FOR_INPUT */,
                confidence: 0.8,
            };
        }
        // ── STILL_WORKING patterns (tool results mid-stream) ────────────────────
        const WORKING_PAT = /^(running|reading|writing|editing|searching|analyzing|calling|creating|updating|deleting|fetching|generating|thinking|preparing|processing|executing|building|checking|verifying)/i;
        if (WORKING_PAT.test(text.trimStart())) {
            return { category: "STILL_WORKING" /* LastMessageCategory.STILL_WORKING */, confidence: 0.7 };
        }
        return { category: "UNKNOWN" /* LastMessageCategory.UNKNOWN */, confidence: 0 };
    }
    // ─── Private ───────────────────────────────────────────────────────────────
    async _poll() {
        if (!this._running) {
        ...
        const progressTexts = items
            .filter((i) => i.type === "PROGRESS_TEXT" /* DOMEvent.PROGRESS_TEXT */ ||
            i.type === "TEXT_CHANGE" /* DOMEvent.TEXT_CHANGE */ ||
            i.type === "PROGRESS_POLL" /* DOMEvent.PROGRESS_POLL */)
            .map((i) => i.text)
            .filter((t) => !!t);
        const isUserTyping = items.some((i) => i.type === "USER_TYPING" /* DOMEvent.USER_TYPING */);
        ...
        const isGenerating = items.some((i) => i.type === "SPINNER" /* DOMEvent.SPINNER */ ||
            i.type === "STOP_BTN_APPEARED" /* DOMEvent.STOP_BTN_APPEARED */ ||
            i.type === "PROGRESS_TEXT" /* DOMEvent.PROGRESS_TEXT */ ||
            i.type === "TEXT_CHANGE" /* DOMEvent.TEXT_CHANGE */ ||
            i.type === "PROGRESS_POLL" /* DOMEvent.PROGRESS_POLL */);
        ...
        const hasBtnGone = items.some((i) => i.type === "STOP_BTN_GONE" /* DOMEvent.STOP_BTN_GONE */);
        const hasOnlyPostCompletionActivity = items.every((i) => i.type === "TEXT_CHANGE" /* DOMEvent.TEXT_CHANGE */ ||
            i.type === "NEW_RESPONSE" /* DOMEvent.NEW_RESPONSE */ ||
            i.type === "STOP_BTN_GONE" /* DOMEvent.STOP_BTN_GONE */);
        const isCompleted = hasBtnGone && (!isGenerating || hasOnlyPostCompletionActivity);
        this._callback({
            events: items,
            isGenerating,
            isCompleted,
    ...
    async _tryReconnect() {
        ...
        try {
            ...
            this._cdp = new CDPClient_1.CDPClient(this._cdpPort);
            await this._cdp.connect();
            const result = await this._cdp.evaluate(INJECT_SCRIPT);
            if (result === "INSTALLED" || result === "ALREADY") {
                this._installed = true;
...
exports.ChatDOMWatcher = ChatDOMWatcher;
