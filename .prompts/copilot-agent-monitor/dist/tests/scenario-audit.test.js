...
function decide(input) {
    ...
    if (state === AgentState.RECOVERING) {
        if (input.msSinceRecoveryStart < COOLDOWN_MS.recovering) {
            const remaining = Math.ceil((COOLDOWN_MS.recovering - input.msSinceRecoveryStart) / 1000);
            return {
...
function canDo(lastActionTs, cooldownMs) {
    return Date.now() - lastActionTs >= cooldownMs;
}
// ─── Test Helpers ───────────────────────────────────────────────────────────
function freshCooldowns() {
    return {
        sendContinue: 0,
        openNewChat: 0,
        stopAndNewChat: 0,
...
let passed = 0;
let failed = 0;
function assert(testName, actual, expected, reasoning) {
    if (actual === expected) {
...
console.log("  Flow: LogWatcher → _onLogDelta(SUCCESS) → _stateFromLog() → GENERATING");
console.log("  Monitor: _observeState returns GENERATING (no screenshot needed)");
...
// Incluso si lleva 10 min sin otra actividad
{
    ...
    assert("1b. GENERATING + 10min stall → WAIT (no stall override)", d.action, AgentAction.WAIT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
// ESCENARIO 2: Tarea completada — grace period + nuevo chat
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n─── Escenario 2: COMPLETED — tarea terminó limpiamente ───");
console.log("  Trigger: LogWatcher detecta shouldContinue=false → LOOP_STOPPED");
console.log("  Flow: _onLogDelta(LOOP_STOPPED) → _stateFromLog() → COMPLETED");
console.log("  Monitor resets _lastActivityMs (BUG FIX #3 — prevents false stall)");
...
console.log("  StateMachine: COMPLETED + ≥90s → OPEN_NEW_CHAT + loop prompt\n");
{
    ...
    assert("2a. COMPLETED + 30s → WAIT (grace period)", d.action, AgentAction.WAIT, d.reasoning);
}
{
    const d = decide(baseInput({
        state: AgentState.COMPLETED,
    ...
    assert("2b. COMPLETED + 91s → OPEN_NEW_CHAT", d.action, AgentAction.OPEN_NEW_CHAT, d.reasoning);
}
console.log("  ↳ ActionExecutor path para OPEN_NEW_CHAT:");
console.log("    1. execute(OPEN_NEW_CHAT) → _activeAbort = new AbortController()");
console.log("    2. _executeInner → _openNewChat()");
console.log("    3. ModelManager available? → Yes → _validateZeroX('new-chat')");
console.log("    4.   applyBestZeroX() → gpt-4.1 > gpt-5-mini > first 0x");
console.log("    5.   LM API: send 'Responde SI...' health check → read response");
console.log("    6.   Response contains SI/YES?");
console.log("    7.     YES → CMD_CHAT_NEW → _sleep(1500) [cancellable] → _sendToChat(loopPrompt)");
console.log("    8.     _sendToChat: PREMIUM GUARD checks lastApplied.isFree");
console.log("    9.     isFree=true → send via CLI 'code chat --mode agent ...'");
console.log("    10. _postAction: openNewChat cooldown=now, newChatCount++, state→RECOVERING");
{
    // Cooldown active
    const d = decide(baseInput({
        state: AgentState.COMPLETED,
    ...
    assert("2c. COMPLETED + 91s + cooldown → WAIT", d.action, AgentAction.WAIT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  StateMachine: STALLED_SOFT → SEND_CONTINUE ('continuar' in same chat)\n");
{
    const d = decide(baseInput({
        state: AgentState.IDLE,
        mssSinceLastActivity: 310_000,
    }));
    assert("3a. 310s inactivity → SEND_CONTINUE (soft stall)", d.action, AgentAction.SEND_CONTINUE, d.reasoning);
}
...
console.log("    3. ModelManager available → _validateZeroX('continue-same-chat')");
...
console.log("    9. _postAction: sendContinue cooldown, softErrorRetries++, state→RECOVERING");
{
    // Cooldown active
    const d = decide(baseInput({
        state: AgentState.IDLE,
    ...
    assert("3b. Soft stall + cooldown → WAIT", d.action, AgentAction.WAIT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  StateMachine: → OPEN_NEW_CHAT (fresh chat + loop prompt)\n");
{
    ...
    assert("4a. 500s inactivity → OPEN_NEW_CHAT (hard stall)", d.action, AgentAction.OPEN_NEW_CHAT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("    Paso 2: Health validated OK → SWITCH_CHAT_MODEL (next 0x model)");
console.log("    Paso 3: SWITCH on cooldown → CYCLE_MODEL (next 0x + new chat)");
console.log("    Paso 4: Everything on cooldown → WAIT\n");
{
    ...
    assert("5a. Rate limit + no health → VALIDATE_ZERO_X", d.action, AgentAction.VALIDATE_ZERO_X, d.reasoning);
}
console.log("  ↳ ActionExecutor para VALIDATE_ZERO_X:");
console.log("    1. _validateZeroX('new-chat') [default mode]");
console.log("    2. applyBestZeroX() → selects gpt-4.1 (ZERO_X_VISION_FAMILIES[0])");
console.log("    3. If no 0x model → return 'NO_ZERO_X_MODEL_AVAILABLE' → Monitor STOPS");
console.log("    4. LM API: send 'Responde SI si puedes...' → consume response");
console.log("    5. Response contains SI/YES?");
console.log("    6.   YES → CMD_CHAT_NEW → _sleep(1500) → _sendToChat(loopPrompt)");
...
console.log("    8. In BOTH cases: model STAYS on 0x — NEVER switches to 1x");
{
    ...
    assert("5b. Rate limit + health OK → SWITCH_CHAT_MODEL (to next 0x)", d.action, AgentAction.SWITCH_CHAT_MODEL, d.reasoning);
}
console.log("  ↳ ActionExecutor para SWITCH_CHAT_MODEL (v1.3.6 fix):");
console.log("    1. _switchChatModel() → ModelManager.zeroXModels (NOT raw _modelPool)");
console.log("    2. Round-robin: find current model in zeroXModels → (idx+1) % length");
console.log("    3. applyModel(nextZeroX) → changeModel command → _lastAppliedModel = nextZeroX");
...
console.log("    5. PREMIUM GUARD: _lastAppliedModel.isFree MUST be true");
{
    ...
    assert("5c. Rate limit + switch cooldown → CYCLE_MODEL", d.action, AgentAction.CYCLE_MODEL, d.reasoning);
}
console.log("  ↳ ActionExecutor para CYCLE_MODEL (v1.3.6 fix):");
console.log("    1. _cycleModel() → ModelManager.zeroXModels (same 0x-only fix)");
...
console.log("    3. _sendToChat(loopPrompt) in new chat");
{
    ...
    assert("5d. Rate limit + all cooldowns → WAIT", d.action, AgentAction.WAIT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  Flow: retry 1 → retry 2 → health check → retry 3 → OPEN_NEW_CHAT\n");
{
    const d = decide(baseInput({
        state: AgentState.ERROR_HARD,
        softErrorRetries: 0,
    }));
    assert("6a. Hard error retry 0 → SEND_CONTINUE (retry 1/3)", d.action, AgentAction.SEND_CONTINUE, d.reasoning);
}
{
    const d = decide(baseInput({
        state: AgentState.ERROR_HARD,
    ...
    assert("6b. Hard error retry 1 → SEND_CONTINUE (retry 2/3)", d.action, AgentAction.SEND_CONTINUE, d.reasoning);
}
{
    const d = decide(baseInput({
        state: AgentState.ERROR_HARD,
    ...
    assert("6c. Hard error retry 2 + no health → VALIDATE_ZERO_X", d.action, AgentAction.VALIDATE_ZERO_X, d.reasoning);
}
{
    const d = decide(baseInput({
        state: AgentState.ERROR_HARD,
    ...
    assert("6d. Hard error retry 2 + health OK → SEND_CONTINUE (retry 3/3)", d.action, AgentAction.SEND_CONTINUE, d.reasoning);
}
{
    ...
    assert("6e. Hard error 3 retries + no health → VALIDATE_ZERO_X first", d1.action, AgentAction.VALIDATE_ZERO_X, d1.reasoning);
}
{
    // Con health OK + 3 retries → ahora sí OPEN_NEW_CHAT
    const d2 = decide(baseInput({
        state: AgentState.ERROR_HARD,
        softErrorRetries: 3,
        health: { validatedOk: true, validatedAt: Date.now() - 60_000 },
    }));
    assert("6f. Hard error 3 retries + health OK → OPEN_NEW_CHAT", d2.action, AgentAction.OPEN_NEW_CHAT, d2.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  Flow: STOP current gen → new chat (highest priority after GENERATING)\n");
{
    const d = decide(baseInput({ state: AgentState.ERROR_CONTEXT }));
    assert("7a. Context full → STOP_AND_NEW_CHAT", d.action, AgentAction.STOP_AND_NEW_CHAT, d.reasoning);
}
...
console.log("    6. _postAction: both stopAndNewChat + openNewChat cooldowns stamped");
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  Flow: FOCUS_VSCODE → bring to foreground\n");
{
    ...
    assert("8a. VSCODE_HIDDEN → FOCUS_VSCODE", d.action, AgentAction.FOCUS_VSCODE, d.reasoning);
}
...
console.log("  Trigger: _postAction sets state→RECOVERING after any successful action");
console.log("  Flow: WAIT for 120s → then re-evaluate\n");
{
    ...
    assert("9a. RECOVERING + 30s → WAIT (in window)", d.action, AgentAction.WAIT, d.reasoning);
}
{
    const d = decide(baseInput({
        state: AgentState.RECOVERING,
    ...
    assert("9b. RECOVERING expired + soft stall → SEND_CONTINUE (falls through)", d.action, AgentAction.SEND_CONTINUE, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
// ESCENARIO 10: Model switch suggested by Copilot
// ─────────────────────────────────────────────────────────────────────────────
console.log("\n─── Escenario 10: ERROR_SWITCH_MODEL — Copilot pide cambiar ───");
console.log("  Trigger: Screenshot/log detects 'switch model' / 'not available'");
{
    const d = decide(baseInput({ state: AgentState.ERROR_SWITCH_MODEL }));
    assert("10a. Switch model suggested → SWITCH_CHAT_MODEL", d.action, AgentAction.SWITCH_CHAT_MODEL, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("\n─── Escenario 11: IDLE — todo normal, actividad reciente ───");
{
    ...
    assert("11a. Normal idle 30s → WAIT", d.action, AgentAction.WAIT, d.reasoning);
}
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("    if (lastApplied && !lastApplied.isFree && no authorization) → BLOCK + log error");
console.log("    if (lastApplied && !lastApplied.isFree && authorization matches) → allow ONE send");
...
console.log("    4. _sendToChat() consumes that authorization + records the 1x prompt");
...
console.log("    ✅ Recovery NEW chat → open chat → UI picker 1x → send loop prompt");
...
console.log("    ⚠️  Manual palette switch to 1x → allowed but logged + warned");
console.log("    ⚠️  If user changes model via VS Code native picker → lastApplied stays old");
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("    1. CostGuard.check() returns {allowed:false, justTripped:true}");
...
console.log("  No aplica a: STOP_AND_NEW_CHAT, VALIDATE_ZERO_X, CYCLE_MODEL, SWITCH_CHAT_MODEL, FOCUS_VSCODE");
console.log("  Flow:");
...
console.log("    5. Screenshot quota exhausted → ALLOW (never block permanently)");
...
console.log("    1. fs.watch(logPath, {persistent:false}) → fires within ms on write");
...
console.log("    5. Heartbeat 10s → _resolveAndWatch() checks for log rotation");
...
console.log("    - Monitor puede cancelar acciones mid-flight si detecta evento crítico");
console.log("    - Picker navigation (400-600ms each) no bloquea el event loop");
console.log("    - _sleep(1500) after CMD_CHAT_NEW resolves early if cancelled");
// ─────────────────────────────────────────────────────────────────────────────
...
console.log("  Flow:");
console.log("    1. ActionExecutor returns {detail: 'NO_ZERO_X_MODEL_AVAILABLE'}");
...
if (failed > 0) {
    ...
    console.log("   El brain usa 0x para observación/rotación y 1x solo cuando recovery lo selecciona explícitamente desde la UI del chat");
