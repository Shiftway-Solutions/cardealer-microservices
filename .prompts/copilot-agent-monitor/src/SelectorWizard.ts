...
type WizardMessage =
  | { type: "ready" }
  | { type: "navigate"; direction: "next" | "prev" | "skip" | "finish" }
  | { type: "startCapture" }
  | { type: "cancelCapture" }
  | { type: "testSelector"; selector: string }
  | { type: "selectorEdited"; elementId: string; selector: string }
  | { type: "resetAll" }
...
type WizardUpdate =
  | { type: "init"; state: WizardState }
  | { type: "stateUpdate"; state: WizardState }
  | { type: "captureStarted" }
  | {
      type: "captureResult";
      result: {
  ...
  | { type: "testResult"; matched: number; selector: string }
...
interface WizardState {
  ...
  confidences: Record<string, StoredSelector["confidence"]>;
  cdpAvailable: boolean;
  capturing: boolean;
  total: number;
  /** Full command the user must paste in an EXTERNAL terminal to relaunch VS Code with CDP enabled.
   *  Uses process.execPath (the real Electron binary) to bypass the `code` CLI wrapper,
   *  which drops --remote-debugging-port when VS Code is already running. */
  cdpLaunchCmd: string;
}

// ─────────────────────────────────────────────────────────────────────────────

export class SelectorWizard {
  private _panel: vscode.WebviewPanel | null = null;
  private _cdp: CDPClient;
  private _store: SelectorStore;
  ...
  constructor(
    ...
    this._store = new SelectorStore(_context);
    ...
    this._state = {
      ...
      selectors: Object.fromEntries(
        KNOWN_ELEMENTS.map((el) => [el.id, stored[el.id]?.selector ?? ""]),
      ),
  ...
  async open(): Promise<void> {
    ...
    this._panel.webview.onDidReceiveMessage(
      (msg: WizardMessage) => this._handleMessage(msg),
      undefined,
  ...
  private async _handleMessage(msg: WizardMessage): Promise<void> {
    switch (msg.type) {
      case "ready":
      ...
      case "startCapture": {
        ...
        try {
          ...
          const el = KNOWN_ELEMENTS[this._state.currentIndex];
      ...
      case "selectorEdited": {
        ...
        if (KNOWN_ELEMENTS.find((e) => e.id === elementId)) {
      ...
      case "launchCDP": {
        ...
        const sel = await vscode.window.showInformationMessage(
          `✅ Command copied! Terminal.app is opening.\n\n` +
            `Paste the command there, then click "Quit VS Code" — after VS Code restarts, open the wizard again.`,
          { modal: true },
          "Quit VS Code",
          "Later",
        );
      ...
      case "resetAll": {
        ...
        const fresh = new SelectorStore(this._context).snapshot();
        this._state.selectors = Object.fromEntries(
          KNOWN_ELEMENTS.map((el) => [el.id, fresh[el.id]?.selector ?? ""]),
        );
        this._state.confidences = Object.fromEntries(
          KNOWN_ELEMENTS.map((el) => [el.id, "default" as const]),
        );
        this._state.currentIndex = 0;
        this._sendState();
        vscode.window.showInformationMessage(
          "All selectors reset to defaults.",
        );
        break;
      }
    }
  ...
  private _saveAll(): void {
    ...
    vscode.window.showInformationMessage(
      `Selector Wizard: ${KNOWN_ELEMENTS.length} selectors saved! ✅`,
    );
  }
  ...
  private _sendUpdate(update: WizardUpdate): void {
    this._panel?.webview.postMessage(update);
  }

  // ─── Screenshot helper ───────────────────────────────────────────────────────

  private _captureScreen(): Promise<Buffer | null> {
