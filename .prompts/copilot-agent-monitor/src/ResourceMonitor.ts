...
export interface ResourceSnapshot {
  ...
  /** 0–100 */
  diskUsedPct: number;
  /** VS Code Node.js process RSS in MB */
  vscodeProcMb: number;
  /** Sum of all running containers RAM usage in MB */
  dockerTotalMb: number;
  dockerContainers: DockerContainerStats[];
...
export type ResourcePressureCallback = (event: ResourcePressureEvent) => void;
...
export class ResourceMonitor {
  ...
  private _onPressure: ResourcePressureCallback;
  ...
  private _lastFiredAt: Record<ResourcePressure, number> = {
    none: 0,
    ram_warn: 0,
    ram_critical: 0,
    disk_warn: 0,
  ...
  private readonly _cooldownMs = 5 * 60_000; // 5 minutes

  constructor(callback: ResourcePressureCallback, intervalMs = 60_000) {
  ...
  async capture(): Promise<ResourceSnapshot> {
    return this._capture();
  }

  /** Format snapshot as a short string for the status bar */
  static format(snap: ResourceSnapshot): string {
  ...
  private _evaluate(snap: ResourceSnapshot): ResourcePressure {
    if (snap.ramUsedPct >= 100 - RAM_CRITICAL_PCT) return "ram_critical";
    if (snap.diskFreeGb < DISK_CRITICAL_GB) return "disk_critical";
    if (snap.ramUsedPct >= 100 - RAM_WARN_PCT) return "ram_warn";
    if (snap.diskFreeGb < DISK_WARN_GB) return "disk_warn";
    return "none";
  }
  ...
  private _buildMessage(
    ...
    switch (pressure) {
      case "ram_critical":
        return (
          `🚨 RAM CRÍTICA: solo ${Math.round((snap.ramFreeMb / 1024) * 10) / 10} GB libres ` +
          `(VSCode: ${Math.round(snap.vscodeProcMb)}MB` +
          (snap.dockerAvailable
            ? `, Docker: ${Math.round(snap.dockerTotalMb)}MB)`
            : ")")
        );
      case "ram_warn":
        return (
          `⚠️ RAM baja: ${Math.round((snap.ramFreeMb / 1024) * 10) / 10} GB libres ` +
  ...
  private async _capture(): Promise<ResourceSnapshot> {
      ...
      execFile(
        ...
        (err: Error | null, stdout: string) => {
          ...
          for (const line of stdout.trim().split("\n")) {
            ...
            const memUsageMb = ResourceMonitor._parseMemMb(
              memParts[0]?.trim() ?? "0",
            );
            const memLimitMb = ResourceMonitor._parseMemMb(
              memParts[1]?.trim() ?? "0",
            );
   ...
   */
  static buildRamActionMessage(snap: ResourceSnapshot): string {
    const lines: string[] = [
    ...
    if (snap.dockerAvailable && snap.dockerContainers.length > 0) {
      const heavy = snap.dockerContainers.filter(
        (c) => c.memUsageMb > DOCKER_HEAVY_MB,
      );
      if (heavy.length > 0) {
        lines.push("");
   ...
   */
  static pruneDockerCache(): Promise<{
    ok: boolean;
    freedMb: number;
    detail: string;
  }> {
        ...
        (err: Error | null, stdout: string) => {
          if (err) {
            resolve({
              ok: false,
              freedMb: 0,
              detail: `docker prune failed: ${err.message}`,
            });
            return;
          }
          // Parse "Total reclaimed space: 2.5GB"
          const match =
            /Total reclaimed space:\s*([0-9.]+)\s*(B|KB|MB|GB|TB)/i.exec(
              stdout,
            );
          const freedMb = match
            ? ResourceMonitor._parseMemMb(`${match[1]}${match[2]}`)
            : 0;
          resolve({
            ok: true,
            freedMb,
