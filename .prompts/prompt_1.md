# CORRECCIÓN (Intento 1/3) — Sprint 38: Accesibilidad — Navegación Solo con Teclado
**Fecha:** 2026-04-05 03:07:59
**Fase:** FIX
...
- [x] Fix bugs de S38-T01: Navegación completa con Tab (sin mouse)
  - S38-B1 FIXED: `featured-vehicles.tsx` fallback slug generaba URL desde `title` (incluía trim "LX"). Corregido a usar `{year}-{make}-{model}-{shortId8}` (formato idéntico al backend `GenerateSlug`). 
  - Commit: featured-vehicles.tsx → `slug: v.slug || ${year}-${make}-${model}-${shortId}`

- [x] Ejecutar Gate Pre-Commit (dotnet build + pnpm lint/typecheck/test/build + dotnet test) — PASSED: dotnet 0 err, typecheck ✅, lint 0 errors, 576/576 frontend tests
...
- Fase: FIX
- Estado: COMPLETADO
- Bugs corregidos: S38-B1 — featured-vehicles.tsx slug fallback generaba URL con trim que no matcheaba el endpoint del backend. Fix: generar slug con {year}-{make}-{model}-{shortId8}.
