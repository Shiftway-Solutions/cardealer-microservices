...
TARGET = sys.argv[1] if len(sys.argv) > 1 else "Claude Sonnet 4.6"
...
def main():
    ...
    # Find ALL occurrences of "Sonnet" or model name in the quick pick area
    search_word = TARGET.split()[-1] if len(TARGET.split()) > 1 else TARGET  # e.g. "Sonnet" or "3.5"
    # Try specific identifiers
    target_pos = None
    for search in [TARGET, "Claude Haiku 4.5", "Haiku 4.5", "Haiku", "Claude Sonnet 4.6", "Sonnet 4.6", "Sonnet", "Claude Sonnet 4.5", "Claude Sonnet 4", "GPT-5.4", "GPT"]:
    ...
    if "haiku 4.5" in vtext or "sonnet 4.6" in vtext or "sonnet 4.5" in vtext or "sonnet 4" in vtext or "gpt-5.4" in vtext:
        print(f"VERIFIED: Model changed to {TARGET}!")
    elif "opus" in vtext:
        print(f"WARNING: Model still shows Opus. May need retry.")
    elif "haiku" in vtext:
        print(f"WARNING: Model still shows Haiku. May need retry.")
    else:
